...
export  PWM8_wReadPulseWidth
export _PWM8_wReadPulseWidth
export  PWM8_wReadCounter
export _PWM8_wReadCounter

; The following functions are deprecated and subject to omission in future releases
;
export  wPWM8_ReadPulseWidth    ; deprecated
export _wPWM8_ReadPulseWidth    ; deprecated
export  wPWM8_ReadCounter       ; deprecated
export _wPWM8_ReadCounter       ; deprecated
...
_PWM8_WritePeriod:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[PWM8_PERIOD_LSB_REG], A
   mov   A, X
   mov   reg[PWM8_PERIOD_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
_PWM8_WritePulseWidth:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[PWM8_COMPARE_LSB_REG], A
   mov   A, X
   mov   reg[PWM8_COMPARE_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PWM8_wReadPulseWidth
;
;  DESCRIPTION:
...
;    functions.
;
 PWM8_wReadPulseWidth:
_PWM8_wReadPulseWidth:
 wPWM8_ReadPulseWidth:                           ; this name deprecated
_wPWM8_ReadPulseWidth:                           ; this name deprecated
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   A, reg[PWM8_COMPARE_MSB_REG]
   mov   X, A
   mov   A, reg[PWM8_COMPARE_LSB_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: PWM8_wReadCounter
;
;  DESCRIPTION:
...
;
 PWM8_wReadCounter:
_PWM8_wReadCounter:
 wPWM8_ReadCounter:                              ; this name deprecated
_wPWM8_ReadCounter:                              ; this name deprecated
   ...
   mov   A, reg[PWM8_COMPARE_MSB_REG]            ; Save the Compare register on the stack
   push  A                                          ;
   mov   A, reg[PWM8_COMPARE_LSB_REG]            ;
   push  A                                          ;  -stack frame now 2 bytes-
   PWM8_Stop_M                                   ; Disable the PWM function
   M8C_SetBank1                                     ;
   mov   A, reg[PWM8_INPUT_LSB_REG]              ; save the LSB clock input setting
   push  A                                          ;   on the stack (now 3 bytes) and ...
                                                    ;   hold the clock low:
   mov   reg[PWM8_INPUT_LSB_REG], INPUT_REG_NULL
   M8C_SetBank0                                     ; Extract the Count via DR2 register
   mov   A, reg[PWM8_COUNTER_MSB_REG]            ; DR2 <- DR0 (in the MSB block)
   mov   A, reg[PWM8_COMPARE_MSB_REG]            ; Stash the Count MSB on the stack
   push  A                                          ;  -stack frame is now 4 bytes
   mov   A, reg[PWM8_COUNTER_LSB_REG]            ; DR2 <- DR0 (in the LSB block)
   mov   A, reg[PWM8_COMPARE_LSB_REG]            ; Stash the Count LSB on the stack
   ...
   mov   reg[PWM8_INPUT_LSB_REG], A              ;    and restore it
   M8C_SetBank0                                     ;
   PWM8_Start_M                                  ; Now re-enable the PWM function
