...
export  Counter8_wReadCompareValue
export _Counter8_wReadCompareValue
export  Counter8_wReadCounter
export _Counter8_wReadCounter

; The following functions are deprecated and subject to omission in future releases
;
export  wCounter8_ReadCompareValue  ; deprecated
export _wCounter8_ReadCompareValue  ; deprecated
export  wCounter8_ReadCounter       ; deprecated
export _wCounter8_ReadCounter       ; deprecated
...
_Counter8_WritePeriod:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[Counter8_PERIOD_LSB_REG], A
   mov   A, X
   mov   reg[Counter8_PERIOD_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
_Counter8_WriteCompareValue:
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   reg[Counter8_COMPARE_LSB_REG], A
   mov   A, X
   mov   reg[Counter8_COMPARE_MSB_REG], A
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Counter8_wReadCompareValue
;
;  DESCRIPTION:
...
;    functions.
;
 Counter8_wReadCompareValue:
_Counter8_wReadCompareValue:
 wCounter8_ReadCompareValue:                     ; this name deprecated
_wCounter8_ReadCompareValue:                     ; this name deprecated
   RAM_PROLOGUE RAM_USE_CLASS_1
   mov   A, reg[Counter8_COMPARE_MSB_REG]
   mov   X, A
   mov   A, reg[Counter8_COMPARE_LSB_REG]
   RAM_EPILOGUE RAM_USE_CLASS_1
   ret
...
.SECTION
;-----------------------------------------------------------------------------
;  FUNCTION NAME: Counter8_wReadCounter
;
;  DESCRIPTION:
...
;        functions.
;
 Counter8_wReadCounter:
_Counter8_wReadCounter:
 wCounter8_ReadCounter:                          ; this name deprecated
_wCounter8_ReadCounter:                          ; this name deprecated
   ...
   mov   A, reg[Counter8_COMPARE_MSB_REG]        ; Save the Compare register on the stack
   push  A                                          ;
   mov   A, reg[Counter8_COMPARE_LSB_REG]        ;
   push  A                                          ;   -stack frame now 2 bytes-
   mov   A, reg[Counter8_CONTROL_LSB_REG]        ; Save CR0 (running or stopped state)
   push  A                                          ;   -stack frame now 3 bytes-
   Counter8_Stop_M                               ; Disable (stop) the Counter if running
   M8C_SetBank1                                     ;
   mov   A, reg[Counter8_INPUT_LSB_REG]          ; save the LSB clock input setting
   push  A                                          ;   on the stack (now 4 bytes) and ...
                                                    ;   hold the clock low:
   mov   reg[Counter8_INPUT_LSB_REG], INPUT_REG_NULL
   M8C_SetBank0                                     ; Extract the Count via DR2 register
   mov   A, reg[Counter8_COUNTER_MSB_REG]        ; DR2 <- DR0 (in the MSB block)
   mov   A, reg[Counter8_COMPARE_MSB_REG]        ; Stash the Count MSB on the stack
   push  A                                          ;   -stack frame is now 5 bytes
   mov   A, reg[Counter8_COUNTER_LSB_REG]        ; DR2 <- DR0 (in the LSB block)
   mov   A, reg[Counter8_COMPARE_LSB_REG]        ; Stash the Count LSB on the stack
   ...
   mov   reg[Counter8_INPUT_LSB_REG], A          ;   and restore it
   M8C_SetBank0                                     ; Now re-enable (start) the counter
   mov   A, [X+bOrigControlReg]                     ;   if it was running when
   mov   reg[Counter8_CONTROL_LSB_REG], A        ;   this function was first called
